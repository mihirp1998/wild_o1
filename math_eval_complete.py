...
parser.add_argument("--eval_group", type=int, default=0, help="Checkpoint to evaluate")
...
test_dataset = None
for category in ['prealgebra']:#, 'algebra', 'intermediate_algebra', 'geometry', 'number_theory', 'precalculus', 'counting_and_probability']:
    ...
    print(len(test_data_transformed))
    if test_dataset is None:
        test_dataset = Dataset.from_list(test_data_transformed)
    else:
        test_dataset = concatenate_datasets([test_dataset, Dataset.from_list(test_data_transformed)])
...
class GenerateSamplesCallback(TrainerCallback):
    def __init__(self, test_dataset, tokenizer, generate_every_n_steps=100, num_samples=5, max_new_tokens=400, eval_group=0):
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        with torch.no_grad():
            ...
            if state.global_step % self.generate_every_n_steps == 0:
                ...
                sample_indices = np.arange(self.eval_group * self.num_samples, (self.eval_group + 1) * self.num_samples)
                ...
                for idx, sample in enumerate(samples):
                    ...
                    print(generated_answer, ground_truth_answer, test_correct, test_total)
...
if args.checkpoint_num == -1:
    ...
    checkpoint_path = f"/grogu/user/lilic/hendrycks-sft-openwebmath/{args.exp_id}/checkpoint-{args.checkpoint_num}"
